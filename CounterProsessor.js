...
function ChengeFonter(){
    const StyleLink = document.getElementById('AppliedStylesheet');
    const NowCSS = StyleLink.getAttribute('href');
    if (NowCSS === "CounterStyle.css"){
        StyleLink.setAttribute('href', "CounterStyle_Gothic.css");
    }
    else if (NowCSS === "CounterStyle_Gothic.css"){
        StyleLink.setAttribute('href', "CounterStyle_Mincho.css");
    }
    else if (NowCSS === "CounterStyle_Mincho.css"){
        StyleLink.setAttribute('href', "CounterStyle.css")
    }
}
document.getElementById('ChangeFont').addEventListener('click',ChengeFonter)
document.getElementById('CreditButton').addEventListener('click', () => {
    alert(`【クレジット】\n使用フォント:Mochiy Pop One、Google Sans、Noto Sans JP、Kaisei Tokumin\n(Google Fontsより)\n使用画像:にじさんじ公式サイト、珠乃井ナナのページより`)
})